...
---------------------------------------------------*/	
	...
	function CleanupBeforeAjax(){		
		...
		tanishaSlider.instances.forEach(slider => slider.off());
		tanishaSlider.instances = [];
	}
		...
		function initOnFirstLoad() {
			...
			imagesLoaded('body', function() {
				...
				setTimeout(function(){
					...
					gsap.set($(".showcase-gallery .tanisha-slider .slide-inner"), { opacity: 0 });
					gsap.to($(".showcase-gallery .tanisha-slider .tanisha-slide .slide-inner"), { duration: 2, opacity: 1, delay: 0.3, ease: Power4.easeOut });
					
					var gallerySlideClasses = [".tanisha-slide-prev-two", ".tanisha-slide-prev", ".tanisha-slide-active", ".tanisha-slide-next", ".tanisha-slide-next-two"];
					
					gallerySlideClasses.forEach(function(gallerySlideClass, index) {
						var gallerySlide = $(".showcase-gallery .tanisha-slider " + gallerySlideClass + " .slide-inner");
					...
					gsap.set($("#filters-wrapper, .tanisha-pagination, .cp-button-prev, .cp-button-next, .progress-info, footer .link-text"), {opacity:0});
					gsap.to($("#filters-wrapper, .tanisha-pagination, .cp-button-prev, .cp-button-next, .progress-info, footer .link-text"), {duration: 0.4, opacity:1, delay:0.8, ease:Power3.easeOut});
...
---------------------------------------------------*/	
	...
	function ShowcasePortfolio() {
		...
		if( $('.showcase-portfolio').length > 0 ){
			...
			function filter() {
				var state = Flip.getState('.tanisha-item');
				var projects = document.querySelectorAll('.tanisha-item');
				var startHeight = gsap.getProperty(".showcase-portfolio", "height");
				
				var filters = document.querySelectorAll('.filter-option.is_active');
				var parallaxItems = document.querySelectorAll('.showcase-portfolio .tanisha-item.vertical-parallax .slide-inner');
			...
			if (!isMobile()) {	
				...
				$(".showcase-portfolio .tanisha-item .slide-inner").on('mouseenter', function() {
					$('#ball p').remove();
					var $this = $(this);			
					gsap.to('#ball', {duration: 0.3, borderWidth: '2px', scale: 1.4, borderColor:"rgba(255,255,255,0)", backgroundColor:"rgba(255,255,255,0.1)"});
					gsap.to('#ball-loader', {duration: 0.2, borderWidth: '2px', top: 2, left: 2});
...
---------------------------------------------------*/	
	...
	function ShowcaseGallery() {
		...
		if( $('.showcase-gallery').length > 0 ){
			...
			slider = new tanishaSlider('.showcase-gallery', { 
				...
				on: {	
					init : function(slide) {
						...
						if ($('body').hasClass("show-loader")) {
							...
							imagesLoaded('body', function() {
								...
								gsap.to($(".showcase-gallery .tanisha-slider .tanisha-slide .slide-inner"), { duration: 0.7, opacity: 1, delay: 0.4, ease: Power2.easeOut });
								...
								gsap.to($("#filters-wrapper, .tanisha-pagination, .cp-button-prev, .cp-button-next, .progress-info, footer .link-text"), {duration: 0.5, opacity:1, delay:0.4, ease:Power2.easeOut});
					...
					slideLeaveViewport : function( slide ) {						
						gsap.set($('.tanisha-slider div:not(.tanisha-slide-visible) .slide-events'), { x: "" });						
					},
			...
			const showGrid = () => {
				...
				gsap.to($("footer .link-text, .tanisha-pagination, .progress-info, #filters-wrapper"), {duration: 0.3, opacity:0, y:30, stagger:-0.05, ease:Power2.easeInOut});
				...
				gsap.set(currentImage.closest(".tanisha-slide"), {zIndex: 0});
				...
				gsap.to(".tanisha-slider .tanisha-slide .trigger-item", { duration: 1, opacity:0, scale:0.7, ease:Power2.easeOut });
			...
			const hideGrid = () => {
				...
				gsap.to(".tanisha-slider .tanisha-slide .trigger-item", { duration: 0.5, opacity:1, scale:1, delay:0.2, ease: 'power3.inOut' });				
				...
				Flip.from(flipstate1, {
					...
					onComplete: function() {
						...
						const triggeredItem = document.querySelector('.tanisha-slide.triggered-item');
						if( triggeredItem != null ){							
							triggeredItem.classList.remove('triggered-item');
						}
						
						const tanishaSlides = document.querySelectorAll('.tanisha-slide');
						tanishaSlides.forEach(slide => {
						  	slide.style.zIndex = '';
				...
				gsap.to($("footer .link-text, .tanisha-pagination, .progress-info, #filters-wrapper"), {duration: 0.3, opacity:1, y:0, stagger:0.05, delay:0.4, ease:Power2.easeInOut});
				gsap.to('#ball', {duration: 0.2, borderWidth: '4px', scale:0.5, borderColor:'#999999', backgroundColor:'transparent'});
				gsap.to('#ball-loader', {duration: 0.2, borderWidth: '4px', top: 0, left: 0});
				$("#ball").removeClass("with-blur");
				$('#ball p').remove();
			...
			const getDOMElements = () => {
				...
				const items = [];
				[...document.querySelectorAll('.tanisha-slide')].forEach(item => {
					items.push(new Item(item));
				});
						
				// Cloned items
				const itemsCloned = [];
				[...document.querySelectorAll('.tanisha-slide-clone')].forEach(item => {
				...
				return {					
					items: items,					
					grid: gridItems,					
					currentItem: new Item( document.querySelector('.tanisha-slide.triggered-item') ),
			...
			const initEvents = () => {
				...
				items.forEach((triggerItem) => {
					...
					triggerItem.addEventListener('click', (event) => {
						...
						if( $('.showcase-gallery').length > 0 ){
							...
							event.currentTarget.closest('.tanisha-slide').classList.add('triggered-item');
							showGrid();
						}
					});
				...
				function moveThumbGrid( direction = "next" ){
					...
					if(document.body.classList.contains("grid-open")) {
						...
						// Get all the slides
						let slides = document.querySelectorAll('.tanisha-slide');
						...
						$('.tanisha-slide').removeClass('triggered-item');
			...
			if (!isMobile()) {
				...
				if ($('.showcase-gallery').hasClass("tilt-gallery")) {
					...
					function callTiltSlider(e) {
						moveItSlider(e, '.tanisha-slider-viewport', 30);
					}
				...
				var dragWrapper = $('.tanisha-slider');
				...
				$('.tanisha-slider').on('mouseup touchend', function() {
				...
				$(".showcase-gallery.preview-mode-enabled .tanisha-slide .slide-inner").on('mouseenter', function() {	
